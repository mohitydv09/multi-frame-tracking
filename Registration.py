...
def align_image(template, target, A):
    ...
    while iter_counter<iter_threshold:
        # print("Alignment in progress... Iter No is : ", iter_counter,"/5000")
        ...
        if iter_counter < 500:
            del_p = (np.linalg.inv(hessian) @ F) * 20
        elif iter_counter < 2000:
            del_p = (np.linalg.inv(hessian) @ F) * 10
        elif iter_counter < 3000:
            del_p = (np.linalg.inv(hessian) @ F) * 2
        elif iter_counter < 4000:
            del_p = (np.linalg.inv(hessian) @ F) * 1
        else:
            del_p = (np.linalg.inv(hessian) @ F) * 0.1
...
if __name__ == '__main__':
    template = cv2.imread('./MY_template.jpeg', 0)  # read as grey scale image
    target_list = []
    for i in range(4):
        target = cv2.imread('./my_target{}.jpeg'.format(i+1), 0)  # read as grey scale image
    ...
    plt.imshow(img_warped, cmap='jet', vmin=0, vmax=255)
    plt.axis('off')
    plt.show()
